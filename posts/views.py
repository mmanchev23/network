...
def following(request, username):
    if request.method == 'GET':
        ...
        if not follows:
            return render(request, 'posts/following.html', {'message': "Opps! You don't follow anybody."})

        paginator = Paginator(posted, 10)
        page_number = request.GET.get('page')
        page_obj = paginator.get_page(page_number)

        return render(request, 'posts/following.html', {'page_obj':page_obj})
...
def config(request, username):
    ...
    if request.method == 'GET':
        ...
        if profile.username == user.username:
            return render(request, "posts/config.html", {'profile': profile})
        else:
            return redirect("index")
        ...
        if not email_already or profile.email == email:
            ...
            return render(request, "posts/config.html", {'profile': profile, 'message': '*Email already taked.'})
        profile.save()
